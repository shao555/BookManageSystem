...
<%--    <script type="text/javascript" src="${pageContext.request.contextPath}/js/show.js"></script>--%>
    <script type="text/javascript">
        ...
        function returnBook(bookID) {
            ...
            if (flag == true) {
                alert("归还成功")
                location.href = "/book/bookc/returnBook.do?book_ID=" + bookID;
            }
        }
...
<form action="${pageContext.request.contextPath}/bookc/findByLendedBName" method="post" id="mytab">
    <p align="center">
        书名<input type="text" id="book_Name" name="book_Name"> <input type="submit" id="submit" value="查询"></p>
</form>

<p align="center">
    <a href="${pageContext.request.contextPath}/bookc/show"><input type="button"
                                                               value="可借图书"></a>
...
<%--                </td>--%>
                <td><input type="button" value="归还" onclick="returnBook(${b.book_ID})"></td>
            </tr>
        </c:forEach>
...
<ul class="pagination">
    <li><a href="${pageContext.request.contextPath}/bookc/lended?page=1"><input type="button" value="首页"></a></li>
    <li <c:if test="${pageInfo.pageNum==1}">class="disabled"</c:if>><a
            href="${pageContext.request.contextPath}/bookc/lended?page=${pageInfo.pageNum-1 }"><input type="button" value="上一页"></a></li><c:forEach begin="1" end="${pageInfo.pages}" step="1" var="pageNo">
    <li <c:if test="${pageInfo.pageNum==pageNo}">class="active"</c:if>>
        <a href="${pageContext.request.contextPath}/bookc/lended?page=${pageNo}"><input type="button" value="${pageNo}"></a></li>
</c:forEach>
    <li <c:if test="${pageInfo.pageNum==pageInfo.pages}">class="disabled"</c:if>><a
            href="${pageContext.request.contextPath}/bookc/lended?page=${pageInfo.pageNum+1}"><input type="button" value="下一页"></a></li>
    <li><a href="${pageContext.request.contextPath}/bookc/lended?page=${pageInfo.pages}"><input type="button" value="尾页"></a></li>
</ul>
